...
<body onload="iniciarParticulas()">
  ...
  ?>
    ...
    <br><br>
     ...
     <h3>Seleccione un PC sobre el que registrar incidencia</h3>

    <form id="formu" name="formu" method="post">
      <table>
        <tr>
          <?php
          $or = ordenadoresReservadosHoy($conn);
          $ord = array();
          foreach ($or as $row) {
            $ord[] = strval($row["id"]);
          }
      ...
      Motivo: <select id="motivo" name="motivo" required>
      			<option>Seleccione una opción</option>
      			<option value="Problemas con la pantalla">Problemas con la pantalla</option>
      			...
      			<option value="Problemas de conectividad">Problemas de conectividad</option>
  ...
  <?php
    ...
    //Comentado porque se debe poder registrar varias incidencias
        ...
        if(empty($pc)){
            "<script>
                 Swal.fire({
                   icon: 'error',
                   title: 'Error',
                   text: 'Debe seleccionar un pc para registrar la incidencia',
                   }).then((xd) => {
                    if(xd){
            ...
            registraIncidencia($conn, $pc, $hoy, $text, $nb,$si);
        }
        echo "<script>
                 Swal.fire({
                   icon: 'success',
